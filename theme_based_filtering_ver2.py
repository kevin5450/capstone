...
def recommend_by_query_words(query_keywords, music_data, model, top_n=5):
    ...
    for song in music_data:
        ...
        results.append((title, artist, sim))
    results.sort(key=lambda x: x[2], reverse=True)
    return results[:top_n]

def recommend_from_query(query, music_data, model, top_n=5):
    keywords = extract_keywords_simple(query)
    if not keywords:
        raise ValueError("입력 문장에서 유효한 키워드가 없습니다.")
    return recommend_by_query_words(keywords, music_data, model, top_n)
...
if __name__ == "__main__":
    ...
    if not keywords:
        ...
        recommendations = recommend_by_query_words(keywords, music_data, model, top_n=10)
        print(f"\n'{user_query}'에 어울리는 추천곡 (사용된 키워드: {', '.join(keywords)}):")
        for i, (title, artist, score) in enumerate(recommendations, start=1):
            print(f"{i}. {title} - {artist} (유사도: {score:.4f})")
